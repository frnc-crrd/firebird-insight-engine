...
Filtro de Facturas:
    ...
    por TIPO_IMPTE = 'C' y CONCEPTO que contenga 'VENTA', usando el SALDO_FACTURA
    historico para asegurar cuadre de pagos parciales.
"""
...
def generar_kpis(
    ...
    for col in ["IMPORTE", "IMPUESTO", "SALDO_FACTURA", "DELTA_MORA", "LIMITE_CREDITO"]:
...
def _calcular_limite_credito(df: pd.DataFrame) -> pd.DataFrame:
    ...
    estatus = df_clientes.set_index("NOMBRE_CLIENTE")["ESTATUS_CLIENTE"] if "ESTATUS_CLIENTE" in df.columns else pd.Series()
